...
- **Endpoints** (`http-server.ts`): admin `POST`/`DELETE /api/contributor/session`; contributor `GET /api/contributor/session` (non-leaky status), `POST /api/contributor/input {text}`, `GET /api/contributor/history?after=N` (raw + `X-Sm-Total-Bytes`), `POST /api/contributor/resize {cols,rows}`, `POST /api/contributor/submit` (commit worktree → push → `gh pr create` via `finalizeJobPr`, emits `job-pr`).
