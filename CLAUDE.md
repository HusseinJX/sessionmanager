...
- **The security spine is structural, not prompt-vetting** (John explicitly can't do per-prompt approval): restricted Claude via `buildContributorCommand()` — `claude --model sonnet --permission-mode acceptEdits --disallowedTools "Bash WebFetch" --allowedTools "Read Edit Write Grep Glob WebSearch TodoWrite" --append-system-prompt <collab-brief> <seed>`. **Deny outranks allow** (verified live on the root+`IS_SANDBOX` VPS: Claude replies "I don't have direct shell access"), so no shell and no arbitrary-host socket — no `git clone`/`tar`/`scp`/`curl` to pull or push the repo wholesale. `--model sonnet` is pinned so sessions never inherit the box default (e.g. the Fable promo). WebSearch still works (server-side via Anthropic API). Belt-and-suspenders in each worktree's `.claude/settings.local.json`: a deny-list (`Bash`, `WebFetch`) **and a `PreToolUse` hook (`deny-escape.py`) that hard-blocks any Read/Edit/Write/Grep/Glob whose resolved path escapes the worktree** — without it, allow-listing Read/Edit/Write let the contributor have Claude read `/root/.config/gh` (GitHub token) or the server `data.json` (admin token); the hook's exit-2 deny can't be overridden by `acceptEdits` or approved by the contributor (verified: gh-token read blocked, in-repo reads still work). Plus **isolated worktree** (branch `contributor/<date>/<proj>-<sess8>`, reuses `worktree.ts`) so Claude sees one branch only, and **no direct push** — code lands only via Submit→PR.
...
- **Device lock + expiry** (`ContributorBinding` in the store): the link is *armed* when an admin starts a session (or `POST /api/contributor/rebind`), then *claimed* by the first device to load the page — `POST /api/contributor/claim` mints a random key, stores its sha256, and sets an httpOnly `sm_contrib_device` cookie. The gate requires every contributor action (beyond `claim`) to carry the matching cookie AND be within the TTL (`SM_CONTRIBUTOR_TTL_DAYS`, default 2, measured from first claim). So a second computer with the same link is "locked to the computer that opened it first", and access auto-dies after ~2 days until re-armed. Admin "Reset device lock" (`/rebind`) re-arms without restarting the session. Binding never overrides the token check — it's an additional factor on top.
