...
- **The security spine is structural, not prompt-vetting** (John explicitly can't do per-prompt approval): restricted Claude via `buildContributorCommand()` — `claude --permission-mode acceptEdits --disallowedTools "Bash WebFetch" --allowedTools "Read Edit Write Grep Glob WebSearch TodoWrite" --append-system-prompt <collab-brief> <seed>`. **Deny outranks allow**, so no shell and no arbitrary-host socket — meaning no `git clone`/`tar`/`scp`/`curl` to pull or push the repo wholesale. WebSearch still works (server-side via Anthropic API). Belt-and-suspenders: a `.claude/settings.local.json` deny-list written into the worktree. Plus **isolated worktree** (branch `contributor/<date>/<proj>-<sess8>`, reuses `worktree.ts`) so Claude sees one branch only, and **no direct push** — code lands only via Submit→PR.
- **Output is a readability pass, not redaction:** `filterForContributor()` strips ANSI/TUI control noise so the raw PTY reads cleanly in the chat pane — code and diffs stay visible on purpose (snippet visibility isn't the concern; convenient bulk-download is). Contributor status (`GET /api/contributor/session`) still never exposes `cwd`/`worktree` path (don't hand out the server's filesystem layout).
- **Endpoints** (`http-server.ts`): admin `POST`/`DELETE /api/contributor/session`; contributor `GET /api/contributor/session` (non-leaky status), `POST /api/contributor/input {data}`, `GET /api/contributor/history` (filtered), `POST /api/contributor/submit` (commit worktree → push → `gh pr create` via `finalizeJobPr`, emits `job-pr`).
- **UI:** `contributor/index.html` (repo root, like `triage/`) — single-file vanilla chat served at `GET /contributor?token=…`. Polls `/session` + `/history` (EventSource can't send the auth header). No terminal, no file tree, no git, no session list.
- **Run it:** set `SM_CONTRIBUTOR_TOKEN` alongside `SM_TOKEN`; give the teammate `https://<host>:7543/contributor?token=<SM_CONTRIBUTOR_TOKEN>`. John (admin) starts their session with `POST /api/contributor/session {project}`. On the VPS also add the **egress allowlist** (`docs/contributor-mode.md`) so a bad approved prompt still can't `curl` out — belt to the tool-deny's suspenders.
